...
class CocoKptDataset(CocoDataset):
    """
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(CocoKptDataset, self).__init__(*arg, **kwargs)
    ...
    def _filter_imgs(self, min_size=32):
        ...
        for i, img_info in enumerate(self.data_infos):
            ...
            annos = self.coco.load_anns(ann_id)
            num_keypoints = 0
            for ann in annos:
    ...
    def _parse_ann_info(self, img_info, ann_info):
        ...
        for i, ann in enumerate(ann_info):
            ...
            if ann.get('iscrowd', False):
                ...
                gt_keypoints.append(np.array(ann['keypoints']).reshape(1, -1, 3))
        ...
        if gt_keypoints:
            gt_keypoints = np.concatenate(gt_keypoints, axis=0, dtype=np.float32)
        else:
            gt_keypoints = np.zeros((0, 17, 3), dtype=np.float32)

        gt_keypoints_ignore = self._get_mask(ann_info, img_info)
        gt_keypoints_ignore = gt_keypoints_ignore.astype(np.uint8)
    ...
    def _get_mask(self, anno, img_info):
        ...
        for obj in anno:
            if 'segmentation' in obj:
                if obj['iscrowd']:
                    rle = maskUtils.frPyObjects(obj['segmentation'],
                                                img_info['height'],
                                                img_info['width'])
    ...
    def results2json(self, results, outfile_prefix):
        if results[0][-1][0].ndim == 3:
            result_files = dict()
            json_results = self.kpt2json(results)
            ...
            return super(CocoKptDataset, self).results2json(results, outfile_prefix)
    
    def kpt2json(self, results):
        bbox_json_results = []
        kpt_json_results = []
