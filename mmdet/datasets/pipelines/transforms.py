...
class Resize:
    ...
    def _resize_kpt(self, results):
        ...
        if 'gt_keypoints_ignore' in results.keys():
            gt_joints_ignore = results['gt_keypoints_ignore'].copy()
            gt_joints_ignore = mmcv.imrescale(gt_joints_ignore,
                                              results['scale'],
                                              interpolation='nearest',
                                              backend=self.backend)
            results['gt_keypoints_ignore'] = gt_joints_ignore[..., None]
    
    def _resize_areas(self, results):
        if 'gt_areas' in results.keys():
            areas = results['gt_areas'].copy()
            areas = areas * results['scale_factor'][0] + areas * results[
                'scale_factor'][1]
    ...
    def __call__(self, results):
        ...
        self._resize_kpt(results)
        self._resize_areas(results)

        # img = np.ascontiguousarray(results['img'], dtype=np.uint8)
        # for kpt in results['gt_keypoints']:
        #     img = self.show_skelenton(img, kpt)
...
class RandomFlip:
    ...
    def kpts_flip(self,
        ...
        gt_keypoints = gt_keypoints[:, self.flip_map]
        flipped = gt_keypoints.copy()
        if direction == 'horizontal':
            w = img_shape[1]
            flipped[:, :, 0] = w - gt_keypoints[:, :, 0] - 1
    ...
    def __call__(self, results):
        ...
        if results['flip']:
            ...
            if 'gt_keypoints' in results.keys():
                results['gt_keypoints'] = self.kpts_flip(results['gt_keypoints'],
                    results['img_shape'])
            
            if 'gt_keypoints_ignore' in results.keys():
                results['gt_keypoints_ignore'] = mmcv.imflip(
                    results['gt_keypoints_ignore'], direction=results['flip_direction'])
...
class Pad:
    ...
    def _pad_keypoints_mask(self, results):
        if 'gt_keypoints_ignore' in results.keys():
            pad_val = self.pad_val.get('mask', 0)
            results['gt_keypoints_ignore'] = mmcv.impad(
                results['gt_keypoints_ignore'], shape=results['pad_shape'][:2], pad_val=pad_val)
    ...
    def __call__(self, results):
        ...
        self._pad_keypoints_mask(results)
        return results
...
class KptRandomCrop:
    ...
    def _crop_data(self, results, crop_size, allow_negative_crop):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            valid_inds = np.where(gt_keypoints[..., 2].sum(-1) > 0)[0]
            if valid_inds.shape[0] == 0:
                return None
            ind = random.choice(valid_inds)
            bbox = results['gt_bboxes'][ind, :]
            bbox[0::2] = np.clip(bbox[::2], 0, img.shape[1])
            bbox[1::2] = np.clip(bbox[1::2], 0, img.shape[0])
            crop_size = np.asarray(crop_size, dtype=np.int32)
            center_yx = (bbox[1] + bbox[3]) * 0.5, (bbox[0] + bbox[2]) * 0.5

            min_yx = np.maximum(np.floor(center_yx).astype(np.int32) - crop_size, 0)
            max_yx = np.maximum(np.asarray(img.shape[:2], dtype=np.int32) - crop_size, 0)
            max_yx = np.minimum(max_yx, np.ceil(center_yx).astype(np.int32))
            offset_h = np.random.randint(min_yx[0], max_yx[0] + 1)
            offset_w = np.random.randint(min_yx[1], max_yx[1] + 1)
        ...
        for key in results.get('bbox_fields', []):
            ...
            kpt_key = self.bbox2keypoint.get(key)
        ...
        # keypoints crop
        if 'gt_keypoints_ignore' in results.keys():
            results['gt_keypoints_ignore'] = \
                results['gt_keypoints_ignore'][crop_y1:crop_y2, crop_x1:crop_x2]
        
        if 'gt_keypoints' in results.keys():
            if len(results['gt_keypoints']) > 0:
                keypoints_offset = np.array([offset_w, offset_h], dtype=np.float32)
                gt_keypoints = results['gt_keypoints'].copy()
                gt_keypoints[..., :2] = gt_keypoints[..., :2] - keypoints_offset
                invalid_idx = (gt_keypoints[..., 0] < 0).astype(np.int8) | \
                              (gt_keypoints[..., 1] < 0).astype(np.int8) | \
                              (gt_keypoints[..., 0] > img_shape[1]).astype(np.int8) | \
                              (gt_keypoints[..., 1] > img_shape[0]).astype(np.int8) | \
                              (gt_keypoints[..., 2] < 0.1).astype(np.int8)
                gt_valid = ~invalid_idx.all(1)
                # results['gt_bboxes'] = results['gt_bboxes'][gt_valid]
                # results['gt_labels'] = results['gt_labels'][gt_valid]
                gt_keypoints[invalid_idx > 0, :] = 0
                # gt_keypoints = gt_keypoints[gt_valid]
                if len(gt_keypoints) == 0 or gt_valid.sum() == 0:
                    return None

            if self.bbox_clip_border:
                gt_keypoints[..., 0] = np.clip(gt_keypoints[..., 0], 0, img_shape[1])
                gt_keypoints[..., 1] = np.clip(gt_keypoints[..., 1], 0, img_shape[0])
            
            results['gt_keypoints'] = gt_keypoints
            assert results['gt_bboxes'].shape[0] == results['gt_keypoints'].shape[0]
    ...
    def __call__(self, results):
        ...
        # if results is not None:
            # cv2.imwrite('crop.jpg', results['img'])
            # for i in results['gt_bboxes']:
            #     rect = cv2.rectangle(results['img'], (int(i[0]),int(i[1])), (int(i[2]),int(i[3])), (0,255,0), 2)
            #     cv2.imwrite('crop.jpg', rect)
            
            # for kpt in results['gt_keypoints']:
            #     results['img'] = self.show_skelenton(results['img'], kpt)
            # cv2.imwrite("keypoints.jpg", results['img'])
        return results
    ...
    def show_skelenton(self, img, kpts, color=(255,128,128), thr=0.01):
        ...
        for sk in skelenton:
            ...
            if pos1[0] > 0 and pos1[1] > 0 and pos2[0] > 0 and pos2[1] > 0 and kpts[sk[0], 2] > thr and kpts[sk[1], 2] > thr:
                cv2.line(img, pos1, pos2, color, 2, 8)
                cv2.circle(img, pos1, radius=4, color=(0, 255, 0))
                cv2.circle(img, pos2, radius=4, color=(0, 255, 0))
        return img
